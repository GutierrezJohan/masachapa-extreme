...
// Removed static categories; now loaded from API
...
const AdminProductCreate: React.FC = () => {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setToast({ open: true, message: 'Producto creado', color: 'success' });
      setTimeout(() => history.replace('/admin/products'), 400);
    } catch (e) {
      setToast({ open: true, message: 'No se pudo crear', color: 'danger' });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <IonPage>
      <IonHeader className="admin-header">
        <IonToolbar>
          ...
          <IonButtons slot="end">
            <IonButton onClick={onSubmit} disabled={submitting} className="success">
          ...
          <IonGrid>
            ...
            <IonRow>
              <IonCol size="12" className="ion-text-right">
                <IonButton onClick={onSubmit} disabled={submitting} className="success">
                  <IonIcon icon={saveOutline} slot="start" /> Guardar Producto
                </IonButton>
              </IonCol>
            </IonRow>
          </IonGrid>
